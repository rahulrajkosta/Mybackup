        ...
        if ($method == 'post' || $method == "POST") {
            ...
            if ($phonestatus == 'locked') {
				...
				if(!empty($mobile_details->device_id)){
                          $rr=$this->send_lock_notification($mobile_details->device_id);
                        }
            }
            if ($phonestatus == 'unlock') {
                ...
                $success = $this->send_notification($title, $body, $device_token, $type, $seller_name, $seller_contact, '', '', $user_number, $coupon_code);
				 if(!empty($mobile_details->device_id)){
                            $rr=$this->send_unlock_notification($mobile_details->device_id);
							// var_dump($rr);
							// die("===#");
                        }
            }
            ...
            $checkstatus = DB::table('mobile_details')->where('user_id', $id)->update(['is_locked' => $is_locked]);
