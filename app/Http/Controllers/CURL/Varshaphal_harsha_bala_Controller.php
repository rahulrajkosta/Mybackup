...
class Varshaphal_harsha_bala_Controller extends BaseController
{
    ...
    public function index($vyear)
    {
		...
		$already=$Varshaphal_harsha_bala_Model->already($where);
       if(empty($already)){
       //require_once 'Astro.php';
       $userId =env("USER_ID");
        $CURLKey =env("CURL_KEY");
       $resourceName = "varshaphal_harsha_bala";
      ...
      foreach($arr as $key=>$val){
          ...
          $arr['year']=$vyear;   
          $store=$Varshaphal_harsha_bala_Model->store($arr);
          if($store){
              return $Varshaphal_harsha_bala_Model->show($store);
          }else{
              return false;
          }
	   }else{
